...
extern int jtck_gpio,jtdi_gpio,jtms_gpio,jtdo_gpio;
...
DPUCHAR jtag_inp(void)
{
    ...
    DPUCHAR ret = 0x80u;
	/* User Specific Code */
    if( digitalRead( jtdo_gpio ) ) tdo = TDO;
...
{
	/* User Specific Code */
	if( outdata & TCK ) digitalWrite( jtck_gpio, HIGH );
		else digitalWrite( jtck_gpio, LOW );
	if( outdata & TDI ) digitalWrite( jtdi_gpio, HIGH );
		else digitalWrite( jtdi_gpio, LOW );
	if( outdata & TMS ) digitalWrite( jtms_gpio, HIGH );
		else digitalWrite( jtms_gpio, LOW );
}
...
void dp_display_value(DPULONG value,DPUINT descriptive)
{
    if ( descriptive == HEX)
    {
        printf("%X",(unsigned long)value);
    }
    else
    {
        printf("%d",(unsigned long)value);
    }
}
