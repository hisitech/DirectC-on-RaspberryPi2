...
void dp_get_dat_support_status(void)
{
    ...
    for (global_ulong1 = 0u;global_ulong1 < 16u ;global_ulong1 ++)
    {
        global_uchar1=(DPUCHAR) dp_get_bytes( Header_ID,
        DAT_SUPPORT_STATUS_OFFSET + global_ulong1,1u);
        if (global_uchar1)
        {
            dat_support_status |= global_uint1;
        }
        global_uint1 <<= 1u;
    }
...
void dp_check_device_ID(void)
{
    ...
    * Therefore, it can be used to store the Data file ID for */
    ...
    global_ulong1 = dp_get_bytes(Header_ID,ID_MASK_OFFSET,4U);
...
void dp_test_reg_015_030_check(void)
{
    ...
    DRSCAN_out(LDVPROP_LENGTH, (DPUCHAR*)DPNULL, &global_uchar1);
    
    
    /* Check for 030 mistmacth */
    if (global_uchar1 == 0x20U)
    {
...
void dp_initialize(void)
{
    ...
    for(global_uchar1 = 0u; global_uchar1 < 7u; global_uchar1++)
    {
        global_buf1[global_uchar1] = global_buf1[global_uchar1+1u];
    }
    #ifdef ENABLE_DISPLAY
    dp_display_text("\r\nFSN: ");
...
void dp_verify_device_info_action(void)
{
    ...
    if (error_code == DPE_SUCCESS)
    {
        ...
        {
            global_ulong1 = dp_get_bytes(ULOCK_ID,0u,3u);
        }
        else 
        {
            global_ulong1 = dp_get_bytes(ULOCK_ID,0u,2u);
            global_ulong1 <<= 12u;
        }
        if (global_ulong1)
        {   
        ...
        if ((device_security_flags & 0x003FFFFFu) != (global_ulong1 & 0x003FFFFFu))
        {
            #ifdef ENABLE_DISPLAY
        ...
        if (error_code == DPE_SUCCESS)
        {
            ...
            if (hardware_interface == GPIO_SEL)
            {
                global_uint1 =  (DPUINT)global_buf2[14] | ((DPUINT)global_buf2[15] << 8u);
            }
            #endif
            #ifdef ENABLE_IAP_SUPPORT                                
            if (hardware_interface == IAP_SEL)
            {
                global_uint1 =  (DPUINT)actual_iap_urow[14] | ((DPUINT)actual_iap_urow[15] << 8u);
            }
            #endif
            
            global_ulong1 = (DPULONG) dp_get_bytes(CHECKSUM_ID, 0u, 2u);
            
            if ((global_ulong1 & 0xFFFFu) != global_uint1)
            {
                #ifdef ENABLE_DISPLAY
            ...
            if (error_code == DPE_SUCCESS)
            {
                ...
                for (global_uint1=4u; global_uint1 < 13u; global_uint1++)
                {
                    global_uchar1 = (DPUCHAR) dp_get_bytes(ACT_UROW_DESIGN_NAME_ID,global_uint1 - 4u, 1u);
                    if (global_uint1 == 12u)
                    {
                        global_buf2[global_uint1] &= 0x3Fu;
                        #ifdef ENABLE_IAP_SUPPORT
                        if (hardware_interface == IAP_SEL)
                        {
                            actual_iap_urow[global_uint1] &= 0x3Fu;
                        }
                        #endif
                    }
                    #ifdef ENABLE_GPIO_SUPPORT                                
                    if (hardware_interface == GPIO_SEL)
                    {
                        if (global_uchar1 != global_buf2[global_uint1])
                        {
                            #ifdef ENABLE_DISPLAY
                    ...
                    if (hardware_interface == IAP_SEL)
                    {
                        if (global_uchar1 != actual_iap_urow[global_uint1])
                        {
                            #ifdef ENABLE_DISPLAY
                            dp_display_text("\r\nUROW Setting Error...");
                            #endif
...
void dp_poll_erase(void)
{
    ...
    IRSCAN_in();
    goto_jtag_state(JTAG_RUN_TEST_IDLE,0u);
    for ( global_ulong1 = 0U; global_ulong1 <= MAX_ERASE_POLL; global_ulong1++ )
    {
        dp_delay(ERASE_POLL_DELAY);
        /* check for ROWBUSY and COLBUSY */
        DRSCAN_out(5u, (DPUCHAR*)DPNULL, &global_uchar1);
        goto_jtag_state(JTAG_RUN_TEST_IDLE,0u);
    ...
    if(global_ulong1 > MAX_ERASE_POLL)
    {
        error_code = DPE_ERASE_ERROR;
    }
    return;
...
void dp_poll_device(void)
{
    ...
    IRSCAN_in();
    goto_jtag_state(JTAG_RUN_TEST_IDLE,0u);
    for ( global_ulong1 = 0U; global_ulong1 <= MAX_PROGRAM_POLL; global_ulong1++ )
    {
        dp_delay(POLL_DELAY);
        ...
        if ((global_uchar1 & 0xbU) == 0U)
        {
            break;
        }
    }
    if(global_ulong1 > MAX_PROGRAM_POLL)
    {
...
void dp_read_silsig(void)
{
    ...
    global_ulong1 =  (DPULONG)global_buf1[0] | ((DPULONG)global_buf1[1] << 8) | ((DPULONG)global_buf1[2] << 16) | ((DPULONG)global_buf1[3] << 24);
    
    #ifdef ENABLE_DISPLAY
    dp_display_text("\r\nExpected SILSIG: ");
    dp_display_value(global_ulong1,HEX);
    #endif
...
void dp_program_urow(void)
{
    ...
    else 
    {
        ...
        {
            if (global_buf1[0] & CORE_ERASE_BITS_BYTE0)
            {
                for (global_uchar1=0U; global_uchar1 < 2U; global_uchar1++)
                {
                    global_buf2[global_uchar1+14U] = (DPUCHAR) dp_get_bytes(CHECKSUM_ID,global_uchar1,1U);
                }
                
                for (global_uchar1=0U; global_uchar1 < 9U; global_uchar1++)
                {
                    global_buf2[global_uchar1 + 4U] = (DPUCHAR) dp_get_bytes(ACT_UROW_DESIGN_NAME_ID,global_uchar1,1U);
                }
            }
            global_buf2[12] |= ((DPUCHAR) (cycle_count << 6));
            global_buf2[13] = (DPUCHAR) (cycle_count >> 2);
            global_buf2[3] |= (GPIO_PROGRAMMING_METHOD << 5) & 0xff;
            global_buf2[3] |= ((ALGO_VERSION & 0xfu) << 1u) & 0xff;
            ...
            global_buf2[2] |= (((ALGO_VERSION >> 4u) & 1u) << 7u) & 0xff;
            global_buf2[3] |= (((ALGO_VERSION >> 5u) & 1u)) & 0xff;
            global_buf2[0] |= (DC_PROGRAMMING_SW << 6u) & 0xff;
            global_buf2[1] |= ((DC_PROGRAMMING_SW >> 2u) & 0x3u) & 0xff;
            global_buf2[1] |= (DC_SOFTWARE_VERSION << 2u) & 0xff;
            global_buf2[2] |= ((DC_SOFTWARE_VERSION >> 6u) & 0x1u) & 0xff;
        }        
    }
    
    for (global_uchar1=0U; global_uchar1 < FROM_ROW_BIT_LENGTH/8U ; global_uchar1++)
    {
        global_buf1[global_uchar1] = 0xffU;
    }
    
    for (global_uchar1=0U; global_uchar1 < 8U; global_uchar1++)
    {
    ...
    if (error_code == DPE_SUCCESS)
    {
        ...
        for ( global_uchar1 = 0U; global_uchar1 < 16U; global_uchar1++ )
        {
            if (global_buf1[global_uchar1] != global_buf2[global_uchar1])
            {
                error_code = DPE_PROGRAM_UROW_ERROR;
                break;
            }
...
void dp_set_aes_mode(void)
{
    opcode = AES_MODE;
    IRSCAN_in();
    DRSCAN_in(0u, AES_MODE_BIT_LENGTH, &global_uchar1);
...
void dp_check_security_settings(void)
{
    ...
    /* global_ulong1 is used as a mask to check if any of the device blocks are protected 
    with a passkey but the data file has no pass key to match */
    global_ulong1 = 0u;
    /* global_uchar1 is used to mask out checking security settings of blocks that are not 
    supported in the data file */
    global_uchar1 = (DPUCHAR) ((dat_support_status & 0x3u) | ((dat_support_status >> 1) & 0x3cu));
    ...
    if (!((global_uchar1 & 0x1u) && (dev_enc_status & 0x1u)))
    {
        global_ulong1 |= ULARD | ULAWE;
    }
    #else
    ...
    if ( !((global_uchar1 & 0x2u) && (dev_enc_status & 0x2u) ))
    {
        global_ulong1 |= ULUFJ | ULUFP;
    }
    #else
    /* If the code of a certain block is disabled, the corresponding security bits are ignored */
    dat_enc_status &= 0xFD;
    ...
    if (!((global_uchar1 & 0x3Cu) && (dev_enc_status & 0x3Cu)))
    {
        global_ulong1 |= ULNR0 | ULNW0 | ULNR1 | ULNW1 | ULNR2 | ULNW2 | ULNR3 | ULNW3;
    }
    #else
    /* If the code of a certain block is disabled, the corresponding security bits are ignored */
    dat_enc_status &= 0xC3;
    ...
    */
    ...
    if (return_bytes == 0u)
    {
        ...
        if  ( ((dev_enc_status & global_uchar1) != (dat_enc_status & global_uchar1)) ||
        ...
        (device_security_flags & global_ulong1) )
        {
...
{
    global_uchar1 = (DPUCHAR) dp_get_bytes(Header_ID,DUAL_KEY_SUPPORT_OFFSET,1U);
    if ( global_uchar1 )
    {
