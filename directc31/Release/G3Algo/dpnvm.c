...
void dp_initialize_access_nvm(void)
{
    ...
    /* global_uchar1 is a global variable used to clock the data into the device with access_nvm instruction */
    ...
    for ( global_ulong1 = 0u; global_ulong1 <= MAX_ATTEMPT_NVM_ACCESS; global_ulong1++ )
    {
        ...
        if ((global_uchar1 & global_uchar1 & 0xF) != 0u )
        {
